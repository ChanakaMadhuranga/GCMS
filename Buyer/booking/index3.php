...
<?php
                            if(isset($u_garbage['fabric_sell']) && in_array('Fabric',$b))
                            {     
                              if($u_garbage['fabric_sell']!== '0')   {            
                            ?>
<tr>

                        <div class="row">
                    <td>   <div class="col"> <?= $u_garbage['u_nic_no']?></div></td> 
                     <td>     <div class="col"> Fabric</div></td> 
                     <td>     <div class="col"> <input type="number" class="form-control pqty" value="<?= $u_garbage['fabric_sell'] ?>"></div></td> 
                     <td>      <div class="col"> <input type="text" class="form-control route" value="<?php echo $u_garbage['route'];?>" readonly></div></td> 
                      ...
                      <td>    <div class="col">  <button class="btn btn-info btn-block text-light addItemBtn3"><i class="fas fa-truck"></i>&nbsp;&nbsp;Add to
                                            List</button></div>
                                            </td> </div>


                            <input type="hidden" class="u_nic_no" value="<?=$u_garbage['u_nic_no']?>">
                            <input type="hidden" class="pname" value="Fabric">
                            <input type="hidden" class="pprice" value="<?=$rowA['Fabric']?>">
                     
                            <input type="hidden" class="pcode" value="<?= $u_garbage['u_nic_no']."Fabric". $u_garbage['fabric_sell'] ?>">
