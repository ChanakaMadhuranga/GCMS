...
while($row_b = mysqli_fetch_assoc($queryb))
      ...
      ?>
            ...
            $date=date("Y-m-d");
                        ...
                        <div class="col"> <input type="date" class="form-control date"></div>
                        <div class="col"> <input type="time" class="form-control time"></div>
                        <div class="col">  <button class="btn btn-info btn-block addItemBtn"><i class="fas fa-truck"></i>&nbsp;&nbsp;Add to
                                            List</button></div></div>


                            <input type="hidden" class="u_nic_no" value="<?=$u_garbage['u_nic_no']?>">
                            <input type="hidden" class="pname" value="Plastic">
                            <input type="hidden" class="pprice" value="20">
                     
                            <input type="hidden" class="pcode" value="<?= $u_garbage['u_nic_no']."Plastic" ?>">
...
<?php
                            if(isset($u_garbage['paper_sell']) && in_array('Paper',$b))
                            {     
                              if($u_garbage['paper_sell']!== '0')   {          
                            ?>
<tr>

                        <div class="row">
                  <td>     <div class="col"> <?= $u_garbage['u_nic_no']?></div></td> 
                      <td>  <div class="col">  Paper</div></td> 
                      <td>  <div class="col"> <input type="number" class="form-control pqty" value="<?= $u_garbage['paper_sell'] ?>"></div></td> 
...
<!--                         <div class="col"> <input type="time" class="form-control time"></div> -->
                    ...
                    <td>    <div class="col">  <button class="btn btn-info btn-block text-light addItemBtn1"><i class="fas fa-truck"></i>&nbsp;&nbsp;Add to
                                            List</button></div>
                                            </td> 
                                            </div>
                            ...
                            <input type="hidden" class="u_nic_no" value="<?=$u_garbage['u_nic_no']?>">
                            <input type="hidden" class="pname" value="Paper">
                            <input type="hidden" class="pprice" value="<?=$rowA['Paper']?>">
                     
                            <input type="hidden" class="pcode" value="<?= $u_garbage['u_nic_no']."Paper".$u_garbage['paper_sell'] ?>">
